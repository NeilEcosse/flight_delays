...
Details on how I handled missing data can be found in **Appendix I - missing data**

The missing data changes were applied using a cleaning script saved at *scripts/data_load_clean.R* - this file also joins the source data tables, and creates some derived fields used in my exploratory analysis (e.g. columns converting Fahrenheit temperatures to Celsius, and columns which summarise continuous data such as wind speed into buckets).

Full details of the cleaning script are shown in the code chunk below.
...
# this uses the table beaufort which I created with information from:
# https://www.weather.gov/jetstream/beaufort_max
...
```{r, warning = F, message = F}
# Load data created by cleaning script
all_data_clean <-
...
Looking at their figures for late **arrivals and departures** at Newark in 2017**^8^**, you can see that there are some important factors which cannot be derived from the dataset used for my report.

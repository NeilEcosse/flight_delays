...
The project brief I was given was:
...
I am using data on departures for Jan-Dec 2017, and the airports in question are:
...
## 2 Overview - number and percentage of delayed flights
...
## 3 Which factors have an impact on flight delays?
...
### 3.1 Significance testing - all variables
...
### 3.2 Significance testing - all variables - all NYC airports

As a comparison, I ran the same model used for Newark for the other airports as well, rejecting variables which were not significant as described in section 3.1 above. 
...
### 3.3 Logistic models - with and without non-weather variable
...
I re-ran my logistic model for Newark (using the significant variables identified above) with and without *number of departures in scheduled departure hour*, to test which was a better model - this is measured with the AIC score: the lower the score, the better the model.
...
## 4 Factors not covered by the model
...
Some of these may indeed be important in a real-world context - severe weather conditions will obviously have an impact, and the number of departures may give useful indications on when the airport is reaching or exceeding capacity.
...
This is also one of the tables supplied with the brief, and contains hourly weather observatons for the three airports.
...
* NA temperature : 25,642 of 26,201 records had NA for temperature - using NOAA min daily temperature instead

* Precipitation: similar amount of NAs as there were for temperature, daily total from NOAA used instead

* NA visibility - dropped records where this was missing (11)
